...
#Function for LogIn
def login(email, password): #Edited by Jay
	count = 0
...
def passwordReset(): #Edited by Jay
...
def deleteCard(email):	#Edited by Jay
...
def updateAddress(email):	#Edited by Jay
	cur.execute("SELECT CreditCard FROM CREDITCARD WHERE email = '%s'" % (email))
...
def addCard(email):	#Edited by Jay
...
def addAddress(email):	#Edited by Jay
	print("Input Address Information: ")
...
def searchFlight(email, password):	#Edited by Jay
...
def book(email, password): #Edited by Jay
...
def bookingReview(email, password): #Edit By Hashaa	| updated by Jay 
...
def bookingCancel(email, password): # Edit by Hashaa | updated by Jay
	cur.execute("SELECT * FROM BOOKING WHERE email = '%s'" % (email))
...
def createAccount(): #Edited by Jay
...
def undirectedFlight(): # Edited by Jorell
        print("Which Airport are you currently at:"+nx.center(g))
...
def main(): #Edited by Jay | Updated undirected flights by Jorell | updated by Hashaa
	user = ""
